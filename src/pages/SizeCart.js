import React from 'react';
import MetaData from '../utils/MetaData';
import BreadCrumb from '../components/BreadCrumb';

const SizeCart = () => {
  return (
    <>
      <MetaData title='Size Chart' />
      <BreadCrumb title='Size Chart' />
      <div className='size-chart home-wrapper-2 p-4'>
        <div className='container-xxl'>
          <div className='row'>
            <h3 className='text-center'>
              <u>Size Chart</u>
            </h3>
            <div className='col-12'>
              <table className='table table-striped caption-top'>
                <caption className='text-center fs-5'>
                  Men's Full Sleeve Shirts Size Chart In India
                </caption>
                <caption>* All Measurements Are In Inches</caption>
                <thead>
                  <tr>
                    <th>Sizes</th>
                    <th>XXXS</th>
                    <th>XXS</th>
                    <th>XS</th>
                    <th>S</th>
                    <th>M</th>
                    <th>L</th>
                    <th>XL</th>
                    <th>XXL</th>
                    <th>XXXL</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>Chest</td>
                    <td>32</td>
                    <td>34</td>
                    <td>36</td>
                    <td>38</td>
                    <td>40</td>
                    <td>42</td>
                    <td>44</td>
                    <td>46</td>
                    <td>48</td>
                  </tr>
                  <tr>
                    <td>Length</td>
                    <td>24</td>
                    <td>25</td>
                    <td>26</td>
                    <td>27</td>
                    <td>28</td>
                    <td>29</td>
                    <td>30</td>
                    <td>31</td>
                    <td>32</td>
                  </tr>
                  <tr>
                    <td>Shoulder</td>
                    <td>12</td>
                    <td>13</td>
                    <td>14</td>
                    <td>15</td>
                    <td>16</td>
                    <td>17</td>
                    <td>18</td>
                    <td>19</td>
                    <td>20</td>
                  </tr>
                  <tr>
                    <td>Sleeve</td>
                    <td>21</td>
                    <td>22</td>
                    <td>23</td>
                    <td>24</td>
                    <td>25</td>
                    <td>26</td>
                    <td>26.5</td>
                    <td>27</td>
                    <td>27.5</td>
                  </tr>
                  <tr>
                    <td>Waist</td>
                    <td>34</td>
                    <td>35</td>
                    <td>36</td>
                    <td>37</td>
                    <td>38</td>
                    <td>40</td>
                    <td>43</td>
                    <td>45</td>
                    <td>50</td>
                  </tr>
                </tbody>
              </table>
              <table className='table table-striped caption-top my-4'>
                <caption className='text-center fs-5 border-top border-2 mt-4'>
                  Men's Half Sleeve Shirts Size Chart In India
                </caption>
                <caption>* All Measurements Are In Inches</caption>
                <thead>
                  <tr>
                    <th>Sizes</th>
                    <th>XXXS</th>
                    <th>XXS</th>
                    <th>XS</th>
                    <th>S</th>
                    <th>M</th>
                    <th>L</th>
                    <th>XL</th>
                    <th>XXL</th>
                    <th>XXXL</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>Chest</td>
                    <td>32</td>
                    <td>34</td>
                    <td>36</td>
                    <td>38</td>
                    <td>40</td>
                    <td>42</td>
                    <td>44</td>
                    <td>46</td>
                    <td>48</td>
                  </tr>
                  <tr>
                    <td>Length</td>
                    <td>24</td>
                    <td>25</td>
                    <td>26</td>
                    <td>27</td>
                    <td>28</td>
                    <td>29</td>
                    <td>30</td>
                    <td>31</td>
                    <td>32</td>
                  </tr>
                  <tr>
                    <td>Shoulder</td>
                    <td>12</td>
                    <td>13</td>
                    <td>14</td>
                    <td>15</td>
                    <td>16</td>
                    <td>17</td>
                    <td>18</td>
                    <td>19</td>
                    <td>20</td>
                  </tr>
                  <tr>
                    <td>Sleeve</td>
                    <td>7.5</td>
                    <td>7.75</td>
                    <td>8</td>
                    <td>8.25</td>
                    <td>8.5</td>
                    <td>9</td>
                    <td>9.25</td>
                    <td>9.75</td>
                    <td>10</td>
                  </tr>

                  <tr>
                    <td>Waist</td>
                    <td>34</td>
                    <td>35</td>
                    <td>36</td>
                    <td>37</td>
                    <td>38</td>
                    <td>40</td>
                    <td>43</td>
                    <td>45</td>
                    <td>50</td>
                  </tr>
                </tbody>
              </table>

              <table className='table table-striped caption-top my-4'>
                <caption className='text-center fs-5 border-top border-2 mt-4'>
                  Men's Pant Size Chart
                </caption>
                <caption>* All Measurements Are In Inches</caption>
                <thead>
                  <tr>
                    <th>Size</th>
                    <th>To Fit Waist ( In )</th>
                    <th>To Fit Inseam ( In )</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>XXS</td>
                    <td>28"</td>
                    <td>28"</td>
                  </tr>
                  <tr>
                    <td>XS</td>
                    <td>30"</td>
                    <td>29"</td>
                  </tr>
                  <tr>
                    <td>S</td>
                    <td>32"</td>
                    <td>30"</td>
                  </tr>
                  <tr>
                    <td>M</td>
                    <td>34"</td>
                    <td>33"</td>
                  </tr>
                  <tr>
                    <td>L</td>
                    <td>36"</td>
                    <td>33"</td>
                  </tr>
                  <tr>
                    <td>XL</td>
                    <td>38"</td>
                    <td>34"</td>
                  </tr>
                  <tr>
                    <td>XXL</td>
                    <td>40"</td>
                    <td>36"</td>
                  </tr>
                </tbody>
              </table>
              <table className='table table-striped caption-top my-4'>
                <caption className='text-center fs-5 border-top border-2 mt-4'>
                  Women Size Chart
                </caption>
                <caption>* All Measurements Are In Inches</caption>
                <thead>
                  <tr>
                    <th>Size</th>
                    <th>To Fit Bust (In)</th>
                    <th>To Fit Waist ( In )</th>
                    <th>To Fit Hip ( In )</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>XXS</td>
                    <td>30"</td>
                    <td>24"</td>
                    <td>33"</td>
                  </tr>
                  <tr>
                    <td>XS</td>
                    <td>32"</td>
                    <td>26"</td>
                    <td>35"</td>
                  </tr>
                  <tr>
                    <td>S</td>
                    <td>34"</td>
                    <td>28"</td>
                    <td>37"</td>
                  </tr>
                  <tr>
                    <td>M</td>
                    <td>36"</td>
                    <td>30"</td>
                    <td>39"</td>
                  </tr>
                  <tr>
                    <td>L</td>
                    <td>38"</td>
                    <td>32"</td>
                    <td>41"</td>
                  </tr>
                  <tr>
                    <td>XL</td>
                    <td>40"</td>
                    <td>34"</td>
                    <td>43"</td>
                  </tr>
                  <tr>
                    <td>XXL</td>
                    <td>42"</td>
                    <td>36"</td>
                    <td>45"</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default SizeCart;
